...
export default function WrongNotesPage() {
  ...
  useEffect(() => {
    // ✅ 전기기사 전용 키값(elec-)으로 변경
    const data = localStorage.getItem("elec-wrong-list");
    if (data) {
  ...
  const nextWrong = () => {
    if (selectedAnswer === currentCorrectNum) {
      ...
      // ✅ 전기기사 전용 키값(elec-)으로 변경
      localStorage.setItem("elec-wrong-list", JSON.stringify(updated));
      ...
      <div style={{ minHeight: "100vh", backgroundColor: "#121212", color: "white", padding: "20px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          ...
          <h2 style={{ 
            ...
            <span style={{ color: "#FF5252", marginRight: 10 }}>Q.</span>{renderTextWithMath(currentQ.question)}
          </h2>
            ...
            {displayOptions.map((opt: any, i: number) => {
              ...
              return (
                <div key={i} onClick={() => (showExplanation ? nextWrong() : handleSelect(num))} style={{ 
                  ...
                  {/* ✅ 오답노트 보기 텍스트에 수식 렌더링 적용 */}
                  {num}. {renderTextWithMath(optText)}
                </div>
              );
            })}
          </div>
          ...
          {showExplanation && (
            <div style={{ marginTop: 30, padding: 25, backgroundColor: "#1E1E1E", borderRadius: 15, border: "1px solid #4FC3F7", animation: "fadeIn 0.3s" }}>
              ...
              <div style={{ lineHeight: 1.6, color: "#ddd", fontSize: `${0.95 * fontSize}rem` }}><strong>[해설]</strong> {renderTextWithMath(currentQ.explanation)}</div>
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#121212", color: "white", padding: "40px 20px" }}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40 }}>
          ...
          {/* ✅ 전기기사 메인 페이지로 이동하도록 경로 변경 */}
          <button onClick={() => router.push("/electric")} style={{ padding: "10px 20px", background: "#333", borderRadius: 8, border: "none", color: "white", cursor: "pointer" }}>🏠 홈으로</button>
        </div>
